...
import { validateClickArgs, validateExtractArgs, validateFindArgs, validateNavigateArgs, validateScreenshotArgs, validateTabsArgs, validateTypeArgs, validateWaitArgs, } from "./validators.js";
import { clickScript, extractScript, findScript, infoScript, navigateScript, screenshotScript, tabsScript, typeScript, waitScript, } from "./scripts.js";
...
export function registerBrowserTools(ctx, runner, screenshotDir) {
    const run = async (script, exec) => {
        const result = await runner.run(script, exec.signal);
        return result.value;
    };
    ctx.tools.register(defineTool({
        name: 'browser_navigate',
        description: 'Navigate the current tab to a URL (or go back/forward in history). '
        ...
        async execute(args, exec) {
            ...
            return run(navigateScript(args), exec);
        },
    }));
    ctx.tools.register(defineTool({
        name: 'browser_info',
        description: 'Current browser state: the attached tab (url/title), all open tabs, and ' +
    ...
    ctx.tools.register(defineTool({
        name: 'browser_tabs',
        description: 'List all open tabs, switch the attached tab, or close a tab. ' +
        ...
        async execute(args, exec) {
            validateTabsArgs(args);
            return run(tabsScript(args), exec);
        },
    }));
    ctx.tools.register(defineTool({
        name: 'browser_find',
        description: 'Find interactive elements in the page accessibility tree by name or role ' +
        ...
        output: {
            ...
            render: (_args, value) => {
                ...
                if (matches.length === 0)
                    return text(['No matching elements found']);
                return text([
        ...
        async execute(args, exec) {
            ...
            return run(findScript(args), exec);
        },
    }));
    ctx.tools.register(defineTool({
        name: 'browser_click',
        description: 'Click at viewport coordinates (`x`/`y`, from `browser_find` withBox results ' +
        ...
        async execute(args, exec) {
            validateClickArgs(args);
            return run(clickScript(args), exec);
        },
    }));
    ctx.tools.register(defineTool({
        name: 'browser_type',
        description: 'Type text or press keys in the browser. With `selector`, fills a framework-' +
        ...
        async execute(args, exec) {
            ...
            return run(typeScript(args), exec);
        },
    }));
    ctx.tools.register(defineTool({
        name: 'browser_extract',
        description: 'Extract content from the current page. Either run an arbitrary JS ' +
        ...
        async execute(args, exec) {
            ...
            return run(extractScript(args), exec);
        },
    }));
    ctx.tools.register(defineTool({
        name: 'browser_screenshot',
        description: 'Capture a PNG of the current viewport (or the full page with `full: true`) ' +
        ...
        async execute(args, exec) {
            validateScreenshotArgs(args);
            return run(screenshotScript(args, screenshotDir), exec);
        },
    }));
    ctx.tools.register(defineTool({
        name: 'browser_wait',
        description: 'Wait for page conditions before the next action: document load, a CSS ' +
        ...
        async execute(args, exec) {
            ...
            return run(waitScript(args), exec);
        },
    }));
}
