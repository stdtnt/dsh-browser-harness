...
export function infoScript() {
    return ENVELOPE('result = {"current": current_tab(), "tabs": list_tabs(), "page": page_info()}', 'result');
}
/** List, switch, or close tabs by target id. */
export function tabsScript(args) {
...
export function findScript(args) {
...
export function clickScript(args) {
    const button = args.button ?? 'left';
    const clicks = args.clicks ?? 1;
    const body = args.nodeId !== undefined
        ? `node_id = ${args.nodeId}
try:
...
y = ${args.y}
click_at_xy(x, y, button=${py(button)}, clicks=${clicks})`;
    return ENVELOPE(`${body}\nresult = {"x": x, "y": y, "page": page_info()}`, 'result');
}
...
export function typeScript(args) {
    ...
    return ENVELOPE(`${body}\nresult = {"ok": True, "page": page_info()}`, 'result');
}
/** Extract text/html/attributes via a CSS selector, or run raw JS. */
export function extractScript(args) {
    ...
    return ENVELOPE(`result = js(${py(expression)})\npage = page_info()\nresult = {"source": {"url": page.get("url"), "title": page.get("title")}, "result": result}`, 'result');
}
/** Capture a viewport (or full-page) PNG; defaults into the screenshot dir. */
export function screenshotScript(args, screenshotDir) {
...
export function waitScript(args) {
    const checks = [];
    if (args.forLoad === true) {
        checks.push(`"load": wait_for_load(timeout=${args.loadTimeout ?? 15})`);
    }
    if (args.selector !== undefined) {
        checks.push(`"element": wait_for_element(${py(args.selector)}, timeout=${args.timeout ?? 10}, visible=${args.visible === true ? 'True' : 'False'})`);
    }
